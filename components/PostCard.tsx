...
export default function PostCard({ post }: Props) {
  ...
  const { currentUser, openDM, deletePost } = useApp();
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all duration-200 overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
          ...
          <Link
            href={`/rides/${post.id}`}
            className="ml-auto flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:text-violet-600 hover:bg-violet-50 transition-all"
          >
            View details
          ...
          {!isOwn && (
            <button
              onClick={() => openDM(post)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-violet-50 text-violet-700 hover:bg-violet-100 border border-violet-200 transition-all"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
          ...
          {isOwn && (
            <div className="ml-auto flex items-center gap-2">
              {confirmDelete && !deleting && (
                <>
                  <span className="text-xs text-rose-500 font-medium">Delete this post?</span>
                  <button
